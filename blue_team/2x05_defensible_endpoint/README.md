...
By the end of this project, you are expected to be able to demonstrate, without referring to any prior project notes:

Cross-Project Synthesis

    The ability to apply Linux hardening (2x00), Windows hardening (2x01), telemetry engineering (2x02), patch management (2x03) and perimeter defense (2x04) as a single integrated workflow on one environment

    The ability to produce a professional handoff package that a peer engineer can verify and operate without further briefing

Defensible Engineering

    How to intake a raw environment and capture its unhardened baseline as structured evidence

    How to define a target state in data (not prose) that every subsequent task can be measured against

    How to compose individual hardening, instrumentation and defense scripts into one end-to-end pipeline that is idempotent and auditable

    How to produce a single machine-readable compliance report covering every control in scope

    How to package telemetry and validation evidence in the exact format Module 3 consumes

Professional Handoff

    How to write a runbook that is executable, not narrative: the runbook is a script or a sequence of commands, not a document

    How to produce a manifest with file hashes so that the receiver can verify the integrity of the handoff

    How to design evaluation criteria that are binary and countable, so that the quality of the handoff is not a matter of taste

Resources

All tools, man pages and references from 2x00 through 2x04 remain applicable. No new technical tooling is introduced in this capstone.

Additional:

Handoff and Packaging

    NIST SP 800-53 Rev.5: System and Services Acquisition (SA) family : Read SA-4 and SA-22 for the "delivered and accepted" concept applied to security controls.

    CIS Controls v8: Implementation Groups : IG1 baseline that every newly onboarded endpoint must meet before production.

Evidence and Validation

    NIST SP 800-137 Rev.1 Draft: Information Security Continuous Monitoring : Reference for producing machine-readable control evidence.
